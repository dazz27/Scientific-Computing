...
def act_height(x, y):
    ...
    return (30 * np.sin(y / 40) * np.cos(x / 40) +
            50 * np.sin(np.sqrt(x * x + y * y) / 40)) - 800
...
def init_samples():
    ...
    grid_x, grid_y = np.mgrid[0:ocean_size:complex(0, num_intervals),
...
def plot(idw_plot_type):
    ...
    if idw_plot_type == 3:
        ...
        mlab.surf(grid_x, grid_y, est_z, colormap='Reds',
